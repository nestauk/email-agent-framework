"""Utility functions for email parsing, formatting, and PDF summarization."""
...
from .prompts import PDF_SUMMARISER_PROMPT
from .schemas import PDFSummary

logger = logging.getLogger(__name__)

load_dotenv(".env")

# Azure OpenAI configuration for supported models.
AZURE_MODELS: Dict[str, Dict[str, str]] = {
    "gpt-5-mini": {
        "azure_endpoint": os.getenv("AZURE_OPENAI_ENDPOINT_MINI"),
        "azure_deployment": os.getenv("AZURE_OPENAI_DEPLOYMENT_MINI", "gpt-5-mini"),
    },
    "gpt-5-nano": {
        "azure_endpoint": os.getenv("AZURE_OPENAI_ENDPOINT_NANO"),
        "azure_deployment": os.getenv("AZURE_OPENAI_DEPLOYMENT_NANO", "gpt-5-nano"),
    },
}


def get_llm(model: str = "gpt-5-mini", temperature: float = 0.0):
    """Return a configured Azure OpenAI chat model."""
    try:
    ...
    if not config["azure_endpoint"]:
        raise ValueError(f"Missing Azure endpoint configuration for '{model}'")
...
def parse_gmail(email_input: dict) -> tuple[str, str, str, str, str, list]:
    ...
    """
    logger.debug("Email input from Gmail: %s", email_input)
...
def extract_text_from_pdf_bytes(pdf_bytes: bytes) -> str:
    ...
    try:
        ...
        logger.info("PDF extraction complete: %d total characters", len(extracted_text))
...
def summarise_pdf(text: str, filename: str, char_limit: int = 10000) -> PDFSummary:
    ...
    if not text:
        logger.warning("PDF '%s' extraction returned empty text", filename)
        return PDFSummary(
            filename=filename,
            original_length=0,
            key_points=[],
    ...
    logger.info("Summarizing PDF '%s' (%d characters)", filename, len(text))
    ...
    logger.debug("PDF summarisation response: %s", raw)
    logger.info("PDF '%s' LLM response: %s", filename, raw[:200])
